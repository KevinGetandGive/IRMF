...
//Glb_Inpt_Init Modify with IRMF
int Glb_Inpt_Init(int data_idx, ftr_mtx_strc& cur_fms, rat_mtx_strc& cur_rms, stttc_strc& cur_ss){
	...
	if (data_idx != 0){
		...
		for (int i = cur_rms.rela_num/2; i < rat_num; i++){
			if (cur_rms.lega_usr[cur_rms.inpt_elem[i].usr_idx] && cur_rms.lega_itm[cur_rms.inpt_elem[i].itm_idx]){
				double tmp_mult = cblas_ddot(MATRIX_RANK, &cur_fms.usr_mtx[ITERATION_NUM][cur_rms.inpt_elem[i].usr_idx][0], 1, &cur_fms.itm_mtx[ITERATION_NUM][cur_rms.inpt_elem[i].itm_idx][0], 1); //need modify
	...
	if (data_idx == 0){
		...
		for (int i = 0; i < rat_num; i++)
			cur_rms.inpt_elem[i].rat = cur_rms.inpt_elem[i].rat - cur_ss.rat_mean;
	    printf("%lf\t%lf\t%lf\n",cur_ss.rat_max,cur_ss.rat_min,cur_ss.rat_mean);
    }
	fclose(stdin);
	return rat_num;
}
